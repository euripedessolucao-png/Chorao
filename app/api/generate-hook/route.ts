...
import { countPoeticSyllables } from "@/lib/validation/syllable-counter-brasileiro" // ✅ CORRIGIDO
...
export async function POST(request: NextRequest) {
  try {
    ...
    let maxSyllables = 10 // fallback seguro
    let minSyllables = 4

    if (config) {
      const rules = config.prosody_rules.syllable_count
      if ("absolute_max" in rules) {
        maxSyllables = Math.min(10, rules.absolute_max) // hooks são mais curtos
        minSyllables = Math.max(3, maxSyllables - 4)
      } else if ("without_comma" in rules) {
        maxSyllables = Math.min(10, rules.without_comma.acceptable_up_to)
        minSyllables = Math.max(3, rules.without_comma.min)
      }
    }
