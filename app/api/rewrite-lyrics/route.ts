...
export default function ReescreverPage() {
  ...
  const getSyllableConfig = (selectedGenre: string) => {
    const config = GENRE_SYLLABLE_CONFIG[selectedGenre as keyof typeof GENRE_SYLLABLE_CONFIG] || GENRE_SYLLABLE_CONFIG.default
    return {
      min: config.min,
      max: config.max,
      ideal: config.ideal
  ...
  const handleApplyChoruses = () => {
    ...
    // ✅ FORMATAÇÃO MELHORADA PARA DETECÇÃO
    const chorusText = selectedChoruses.map((c, index) => {
      const chorus = c.chorus.replace(/\s\/\s/g, "\n")
      return `REFRÃO ${index + 1}:\n${chorus}`
    }).join('\n\n')
    ...
    toast.success(`${selectedChoruses.length} refrão(ões) adicionado(s) aos requisitos!`)
  }
    ...
    try {
      ...
      const response = await fetch("/api/rewrite-lyrics", {
        ...
        body: JSON.stringify({
          ...
          lyrics: originalLyrics, // Letra original para reescrever
          genero: genre, // Gênero para conversão
          tema: theme || "Reescrita", // Tema
          humor: mood || "Adaptado", // Humor
          
          // ✅ REFRÕES E HOOKS
          selectedChoruses: selectedChoruses.map(c => c.chorus), // Refrões selecionados
          additionalRequirements: additionalReqs, // Requisitos com hooks/refrões
          
          // ✅ CONFIGURAÇÕES TÉCNICAS
          universalPolish: universalPolish, // Sistema Universal de Polimento
          syllableTarget: syllableConfig, // Configuração de sílabas
          formattingStyle: formattingStyle, // Formatação performática
          advancedMode: advancedMode, // Modo avançado
      ...
      // ✅ FEEDBACK DO SISTEMA
      if (data.metadata?.preservedChorusesUsed) {
        toast.success("Letra reescrita com refrões preservados!", {
          description: `Sistema Universal aplicou polimento específico para ${genre}`
        })
      } else {
  ...
  const handleApplyHook = () => {
    ...
    // ✅ FORMATAÇÃO CLARA PARA DETECÇÃO
    const hookText = `[HOOK]\n${selectedHook}`
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Aplica polimento específico por gênero, correção automática de rimas e preservação de refrões.
                  </p>
                </div>
              </div>
                ...
                >
                  {isRewriting ? (
                    ...
                    <>
                      ...
                      {universalPolish ? "🎵 Reescrever com Polimento" : "Reescrever Letra"}
                    </>
                  )}
                </Button>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div className="space-y-2">
                  ...
                  {/* ✅ VALIDADOR DE SÍLABAS - COM CONFIGURAÇÃO DO GÊNERO */}
                  <SyllableValidator
                    lyrics={lyrics}
