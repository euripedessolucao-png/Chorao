...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const [key, value] of Object.entries(body)) {
      ...
      if (
        typeof value === "string" &&
        ["sertanejo", "mpb", "funk", "forró", "rock", "pop", "gospel", "piseiro"].some((genre) =>
...
${selectedChoruses.length > 0 ? `REFRÃOS PRESERVADOS:\n${selectedChoruses.join("\n")}` : ""}
...
// Função auxiliar para converter tags para inglês
