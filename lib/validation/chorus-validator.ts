 ...
 */
export function validateChorus(chorus: ChorusOption, genre = "sertanejo"): ChorusValidation {
  const errors: string[] = []
  ...
  const minRichRhymesRatio = rhymeRules.minRichRhymePercentage / 100
  const maxFalseRhymesRatio = rhymeRules.maxFalseRhymePercentage / 100

  if (rhymeAnalysis.richRhymeRatio < minRichRhymesRatio) {
    warnings.push(
      `Poucas rimas ricas (${(rhymeAnalysis.richRhymeRatio * 100).toFixed(0)}% < ${rhymeRules.minRichRhymePercentage.toFixed(0)}%)`,
    )
    score -= 10
  }

  if (rhymeAnalysis.falseRhymeRatio > maxFalseRhymesRatio) {
    warnings.push(
      `Muitas rimas falsas (${(rhymeAnalysis.falseRhymeRatio * 100).toFixed(0)}% > ${rhymeRules.maxFalseRhymePercentage.toFixed(0)}%)`,
    )
    score -= 15
  }
...
function analyzeRhymeQuality(lines: string[], rhymeRules: any) {
  ...
  if (richRhymeRatio >= minRichRhymesRatio) {
    overallQuality = "rich"
  } else if (falseRhymeRatio <= maxFalseRhymesRatio) {
    overallQuality = "poor"
  } else {
...
function classifyRhyme(line1: string, line2: string, rhymeRules: any): "rich" | "poor" | "false" {
  ...
  const normalize = (word: string) => word.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
...
export function validateChorusBatch(choruses: ChorusOption[], genre = "sertanejo"): ChorusBatchResult {
