...
const SPECIAL_WORDS: Record<string, number> = {
  ...
  'pra': 1, 'pro': 1, 'pras': 1, 'pros': 1,
  'tá': 1, 'tô': 1, 'vô': 1, 'pô': 1,
  'dá': 1, 'pé': 1, 'pá': 1, 'mãe': 1,
  'põe': 1, 'vêm': 1, 'têm': 1, 'vai': 1,
  'sou': 1, 'dói': 1, 'fui': 1, 'viu': 1,
  'diz': 1, 'faz': 1, 'luz': 1, 'voz': 1,
  'ninguém': 2, 'alguém': 2, 'ninguem': 2, 'alguem': 2,
  'você': 2, 'café': 2, 'sofá': 2, 'paletó': 3,
  'jiló': 2, 'cipó': 2, 'vatapá': 3, 'acarajé': 4,
  'tem': 1, 'bem': 1, 'sem': 1, 'quem': 1,
  'meu': 1, 'teu': 1, 'seu': 1, 'deus': 1,
  'céu': 1, 'véu': 1, 'chapéu': 2,
};
...
export function countPoeticSyllables(line: string): number {
  ...
  // Remove acentos e caracteres não alfabéticos (mantém espaços)
  let clean = line
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '') // remove acentos
    .replace(/[^a-z\s']/gi, ' ')    // mantém letras, espaços e apóstrofo
    .replace(/\s+/g, ' ')
    .trim()
    .toLowerCase();
