...
export class SyllableSuggestionEngine {
  ...
  private static readonly OPTIONAL_WORDS = [
    "muito", "bem", "tão", "mesmo", "realmente", "sempre", "nunca", "quase", "totalmente",
  ];
  ...
  private static reformulateVerse(verse: string, targetSyllables: number): string | null {
    const patterns = [
      ...
      { regex: /\b(o|a)\s+(meu|seu|nosso|teu)\b/gi, replacer: (_: string, __: string, possessive: string) => possessive },
  ...
  private static simplifyVerse(verse: string, targetSyllables: number): string {
    ...
    const withArticlesRemoved = current.replace(/\b(o|a|os|as)\s+/gi, "").replace(/\s+/g, " ").trim();
